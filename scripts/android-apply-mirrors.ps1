...
function Update-AppTauriBuildGradleFile {
    param(
        [string]$Path,
    ...
    if ($lifecycleMatch.Success) {
        $lifecycleVersion = $lifecycleMatch.Groups[1].Value
    }
...
"@
    ...
    Write-Utf8NoBom -Path $Path -Value $updated
    Write-Output "Updated: app/tauri.build.gradle.kts."
}
...
project(':tauri-android').projectDir = new File(rootDir, "tauri-android-vendor")
"@
...
Update-AppTauriBuildGradleFile -Path $appTauriBuildGradlePath -AppGradlePath $appGradlePath
