...
import { createPluginSettingsStore } from './settingsStore';
import type { PluginSettingsStore, WhybraryPlugin } from './types';

export const officialPlugins: readonly WhybraryPlugin[] = [whyReviewPlugin];

export function createOfficialPluginRegistry(
  settingsStore: PluginSettingsStore = createPluginSettingsStore(),
) {
  const registry = new PluginRegistry(settingsStore);
